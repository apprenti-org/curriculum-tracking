 ...
 * - course-overview.js — courseOverviewData, courseOverviewMap
 */
...
/**
 * Generate course-overview.js bundle
 *
 * Reads course-overview.json (produced by scripts/generate-course-overview.py)
 * and emits a JS bundle exposing courseOverviewData and courseOverviewMap.
 * Skips gracefully if course-overview.json doesn't exist.
 */
function generateOverviewBundle(data, config) {
  const overviewPath = path.join(config.rootDir, 'course-overview.json');
  if (!fs.existsSync(overviewPath)) {
    console.log('    course-overview.json not found — skipping overview bundle');
    return;
  }

  const overview = JSON.parse(fs.readFileSync(overviewPath, 'utf8'));

  let js = '// Auto-generated from course-overview.json — do not edit directly\n';
  js += 'var courseOverviewData = ' + JSON.stringify(overview.courses, null, 2) + ';\n\n';
  js += 'var courseOverviewMap = {};\n';
  js += 'courseOverviewData.forEach(function(c) {\n';
  js += '  courseOverviewMap[c.id] = c;\n';
  js += '  courseOverviewMap[c.name] = c;\n';
  js += '});\n';

  const outputPath = path.join(config.rootDir, 'course-overview.js');
  fs.writeFileSync(outputPath, js, 'utf8');
}
