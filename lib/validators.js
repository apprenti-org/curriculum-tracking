...
function validateSchema(data, config) {
  ...
  data.courses.forEach((course, i) => {
    ...
    // Optional statusConfirmed flag — true when the design/dev status has
    // been manually audited (see #35). Boolean-only.
    if (course.statusConfirmed !== undefined && typeof course.statusConfirmed !== 'boolean') {
      errors.push(
        `courses[${i}] (${course.name}): "statusConfirmed" must be a boolean (got ${typeof course.statusConfirmed})`
      );
    }
  });
