/**
 * Dashboard nav panel — builds sidebar with curricula and course list
 * Depends on: courseData, curriculaData (courses.js), courseOutlines (outlines.js),
 *             courseLookup (data-store.js), courseStatusIcon (formatters.js)
 */

// Curriculum map: populated by buildDashboard(), used by detail-panel.js
var curriculumMap = {};

function buildDashboard() {
    var courses = [].concat(courseData).sort(function(a, b) { return a.name.localeCompare(b.name); });
    ...
    else courses.forEach(function(c) { html += buildNavItemFromCourse(c); });
    html += '</div></div>';
...
function buildCurriculumSummary(name, items, syllabus) {
    ...
    var syllabusLink = syllabus ? '<a href="' + syllabus + '" target="_blank" class="curriculum-syllabus-link"><i class="fa-solid fa-file-alt" style="color:#4caf50;font-size:10px;margin-right:4px;"></i>Syllabus</a>' : '';
