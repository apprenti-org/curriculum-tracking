/**
 * Outline rendering — displays module/lesson/topic/assessment tree
 * Pure rendering functions, no data dependencies beyond the outline object passed in
 */
...
function renderOutline(outlineData) {
    ...
    outlineData.modules.forEach(function(mod, i) {
        ...
        html += '<i class="fa-solid fa-chevron-right module-chevron"></i>';
        html += '<span class="module-name"><i class="fa-solid fa-cube" style="opacity:0.5;margin-right:4px;font-size:11px;"></i>Module ' + (i + 1) + ': ' + (mod.name || mod.title) + '</span>';
        html += '<span class="module-hours">' + mod.hours + 'h &middot; ' + mod.lessons.length + ' lessons</span></div>';
        ...
        mod.lessons.forEach(function(l) {
            ...
            html += '<i class="fa-solid fa-file-lines" style="color:var(--text-muted);font-size:10px;"></i>';
            html += '<span style="color:var(--text-muted);font-size:12px;">Lesson ' + num + '.</span> ' + (l.title || l.name);
            html += '<span class="lesson-hours">' + (l.hours || '') + (l.hours ? 'h' : '') + '</span></div>';
    ...
    if (outlineData.assessment) {
        ...
        html += '<i class="fa-solid fa-clipboard-check" style="margin-right:6px;"></i>' + outlineData.assessment.title + ' (' + outlineData.assessment.hours + 'h)</div>';
...
function renderNoOutline() {
    return '<div class="outline-section"><div class="outline-no-data">' +
        ...
        'Course outline not yet loaded. Ingest the outline to see module and lesson details here.' +
        '</div></div>';
}
